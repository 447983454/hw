...
void FLUID_3D::solvePressure_sse(float* field, float* b, unsigned char* skip)
{
  int x, y, z, index;
  __m128 sse_residual;
  ...
  for (z = 1; z < _zRes - 1; z++, index += 2 * _xRes)
    for (y = 1; y < _yRes - 1; y++, index += 2)
      for (x = 1; x < _xRes - 1;){
        if(x%4==0 && x+3<_xRes - 1){//z>24||z<22
          // if(x ==4)
          // cout<<index<<endl;
          //apply sse
          sse_residual = _mm_loadu_ps(_residual+index);
           _mm_store_ss(_direction+index, sse_residual);
          x+=4; index+=4;
        }else{
          _direction[index] = _residual[index];
          x++; index++; 
